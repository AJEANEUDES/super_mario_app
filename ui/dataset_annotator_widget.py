...
class FullScreenAnnotationDialog(QDialog):
    ...
    def _create_center_panel(self) -> QWidget:
        ...
        self.image_canvas.box_moved.connect(self._on_box_moved)
        self.image_canvas.box_selected.connect(self._on_box_selected)
    ...
    def _create_annotations_panel(self) -> QWidget:
        ...
        # Connexion pour synchroniser la sélection tableau → canvas
        self.annotations_table.cellClicked.connect(self._on_table_row_clicked)
    ...
    def _copy_annotations(self):
        """Copier toutes les annotations du frame courant"""
        count = self.image_canvas.copy_all()
        if count > 0:
            self.status_label.setText(f"📋 {count} annotation(s) copiée(s) - Ctrl+V pour coller, Ctrl+→ pour propager")
            self.status_label.setStyleSheet("color: #4caf50; font-weight: bold;")
        else:
            self.status_label.setText("⚠️ Aucune annotation à copier")
            self.status_label.setStyleSheet("color: #ff9800;")
    
    def _paste_annotations(self):
        """Coller les annotations"""
        if self.image_canvas.get_clipboard_count() == 0:
            self.status_label.setText("⚠️ Rien à coller - Utilisez Ctrl+C d'abord")
            self.status_label.setStyleSheet("color: #ff9800;")
            return
        
        pasted = self.image_canvas.paste()
        if pasted:
            for class_id, x, y, w, h in pasted:
                self.task.add_annotation(class_id, x, y, w, h)
            self._display_frame()
            self.status_label.setText(f"📋 {len(pasted)} annotation(s) collée(s)")
            self.status_label.setStyleSheet("color: #4caf50; font-weight: bold;")
    
    def _propagate_to_next(self):
        """Copier les annotations et les coller sur le frame suivant"""
        # Copier si pas déjà fait
        if self.image_canvas.get_clipboard_count() == 0:
            count = self.image_canvas.copy_all()
            if count == 0:
                self.status_label.setText("⚠️ Aucune annotation à propager")
                self.status_label.setStyleSheet("color: #ff9800;")
                return
        
        # Sauvegarder le frame courant
        self._save_current()
        ...
        if self.current_index < len(self.task.images) - 1:
            ...
            # Coller les annotations
            for box in self.image_canvas.clipboard:
            ...
            count = len(self.image_canvas.clipboard)
            self.status_label.setText(f"✅ {count} annotation(s) propagée(s) → Frame #{self.current_index + 1} - Ajustez les positions!")
            self.status_label.setStyleSheet("color: #4caf50; font-weight: bold;")
        else:
            self.status_label.setText("⚠️ Déjà au dernier frame")
            self.status_label.setStyleSheet("color: #ff9800;")
    
    def _on_box_moved(self, index: int, x: int, y: int, w: int, h: int):
        """Quand une annotation est déplacée/redimensionnée"""
        img = self.task.get_current_image()
        if img and 0 <= index < len(img.boxes):
            box = img.boxes[index]
            box.x = x
            box.y = y
            box.width = w
    ...
    def _on_box_selected(self, index: int):
        """Quand une annotation est sélectionnée sur le canvas"""
        if index >= 0:
            self.annotations_table.selectRow(index)
        else:
            self.annotations_table.clearSelection()
    
    def _on_table_row_clicked(self, row: int, col: int):
        """Quand on clique sur une ligne du tableau → sélectionner sur le canvas"""
        self.image_canvas.select_box(row)
...
class HandlePosition(Enum):
    """Position des poignées de redimensionnement"""
    NONE = auto()
    TOP_LEFT = auto()
    TOP_RIGHT = auto()
    BOTTOM_LEFT = auto()
...
class AnnotationCanvasFullscreen(QLabel):
    """
    Canvas d'annotation avec drag & drop fluide style Canva.
    
    Fonctionnalités:
    - Dessiner de nouvelles annotations
    - Sélectionner une annotation existante (clic)
    - Déplacer une annotation (drag & drop fluide)
    - Redimensionner via les poignées aux coins
    - Copier-coller (Ctrl+C / Ctrl+V)
    """
    
    box_drawn = pyqtSignal(int, int, int, int)  # x, y, w, h
    box_moved = pyqtSignal(int, int, int, int, int)  # index, x, y, w, h
    box_selected = pyqtSignal(int)  # index (-1 si aucun)
    annotations_pasted = pyqtSignal(list)  # Liste de tuples (class_id, x, y, w, h)
    ...
    def __init__(self, parent=None):
        ...
        # Annotations: [(class_id, class_name, color, x, y, w, h), ...]
        self.boxes = []
        ...
        self.dragging = False
        self.resizing = False
        self.resize_handle = HandlePosition.NONE
    ...
    def copy_all(self):
        """Copier toutes les annotations"""
        self.clipboard = [box for box in self.boxes]
    ...
    def paste(self):
        ...
        for box in self.clipboard:
            class_id, class_name, color, x, y, w, h = box
            pasted.append((class_id, x, y, w, h))
        self.annotations_pasted.emit(pasted)
        return pasted
    
    def get_clipboard_count(self) -> int:
        return len(self.clipboard)
    ...
    def _update_display(self):
        ...
        if widget_size.width() <= 20 or widget_size.height() <= 20:
            return
    ...
    def _draw(self):
        ...
        if hasattr(self, 'boxes') and self.boxes:
            for i, box in enumerate(self.boxes):
                self._draw_box(painter, box, i == self.selected_index)
    ...
    def _draw_box(self, painter: QPainter, box: tuple, selected: bool):
        """Dessiner une box"""
        class_id, class_name, color, x, y, w, h = box
        ...
        if selected:
            ...
            brush = QBrush(QColor(color[0], color[1], color[2], 30))
        ...
        # Label
        label_color = QColor(0, 150, 255) if selected else QColor(*color)
        font = QFont("Segoe UI", 9, QFont.Weight.Bold)
        painter.setFont(font)
        metrics = painter.fontMetrics()
        text_w = metrics.horizontalAdvance(class_name) + 10
        text_h = metrics.height() + 4
        
        label_rect = QRect(dx, dy - text_h - 2, text_w, text_h)
        ...
        if selected:
            ...
            handles = [
                ...
                (dx + dw - hs//2, dy - hs//2),
                (dx - hs//2, dy + dh - hs//2),
                (dx + dw - hs//2, dy + dh - hs//2),
                (dx + dw//2 - hs//2, dy - hs//2),
                (dx + dw//2 - hs//2, dy + dh - hs//2),
                (dx - hs//2, dy + dh//2 - hs//2),
                (dx + dw - hs//2, dy + dh//2 - hs//2),
            ]
    ...
    def _display_to_image(self, pos: QPoint) -> QPoint:
    ...
    def _get_box_at(self, img_x: int, img_y: int) -> int:
        """Trouver la box à cette position"""
        for i in range(len(self.boxes) - 1, -1, -1):
            _, _, _, x, y, w, h = self.boxes[i]
            if x <= img_x <= x + w and y <= img_y <= y + h:
                return i
        return -1
    
    def _get_handle_at(self, img_x: int, img_y: int) -> HandlePosition:
        """Trouver la poignée à cette position"""
        if self.selected_index < 0:
            return HandlePosition.NONE
        
        _, _, _, x, y, w, h = self.boxes[self.selected_index]
        hs = int(self.handle_size / self.scale_factor) + 5
        
        # Coins
        if abs(img_x - x) <= hs and abs(img_y - y) <= hs:
            return HandlePosition.TOP_LEFT
        if abs(img_x - (x + w)) <= hs and abs(img_y - y) <= hs:
            return HandlePosition.TOP_RIGHT
        if abs(img_x - x) <= hs and abs(img_y - (y + h)) <= hs:
            return HandlePosition.BOTTOM_LEFT
        if abs(img_x - (x + w)) <= hs and abs(img_y - (y + h)) <= hs:
            return HandlePosition.BOTTOM_RIGHT
        
        # Côtés
        if abs(img_y - y) <= hs and x < img_x < x + w:
            return HandlePosition.TOP
        if abs(img_y - (y + h)) <= hs and x < img_x < x + w:
            return HandlePosition.BOTTOM
        if abs(img_x - x) <= hs and y < img_y < y + h:
            return HandlePosition.LEFT
        if abs(img_x - (x + w)) <= hs and y < img_y < y + h:
    ...
    def _update_cursor(self, pos: QPoint):
        ...
        if not self._is_in_image(pos):
            self.setCursor(Qt.CursorShape.ArrowCursor)
            return
        
        img_pos = self._display_to_image(pos)
        
        # Poignées
        if self.selected_index >= 0:
            handle = self._get_handle_at(img_pos.x(), img_pos.y())
            if handle in (HandlePosition.TOP_LEFT, HandlePosition.BOTTOM_RIGHT):
                ...
                return
            elif handle in (HandlePosition.TOP, HandlePosition.BOTTOM):
                self.setCursor(Qt.CursorShape.SizeVerCursor)
                return
            elif handle in (HandlePosition.LEFT, HandlePosition.RIGHT):
                self.setCursor(Qt.CursorShape.SizeHorCursor)
                return
        
        # Sur une box
        if self._get_box_at(img_pos.x(), img_pos.y()) >= 0:
    ...
    def mousePressEvent(self, event: QMouseEvent):
        ...
        # 2. Clic sur une box?
        box_idx = self._get_box_at(img_pos.x(), img_pos.y())
        if box_idx >= 0:
            self.selected_index = box_idx
            self.dragging = True
            self.drag_start_pos = img_pos
            _, _, _, x, y, w, h = self.boxes[box_idx]
            self.drag_start_box = (x, y, w, h)
            self.setCursor(Qt.CursorShape.ClosedHandCursor)
            self.box_selected.emit(box_idx)
            self._draw()
            return
        
        # 3. Nouvelle annotation
        self.selected_index = -1
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        ...
        if not self.drawing and not self.dragging and not self.resizing:
        ...
        # Déplacement
        if self.dragging and self.drag_start_pos and self.drag_start_box:
            img_pos = self._display_to_image(self._clamp_to_image(pos))
            dx = img_pos.x() - self.drag_start_pos.x()
            dy = img_pos.y() - self.drag_start_pos.y()
            
            orig_x, orig_y, w, h = self.drag_start_box
            new_x = orig_x + dx
            new_y = orig_y + dy
            
            # Limiter
            if self.original_pixmap:
                new_x = max(0, min(new_x, self.original_pixmap.width() - w))
                new_y = max(0, min(new_y, self.original_pixmap.height() - h))
            
            # Mettre à jour la box
            class_id, class_name, color, _, _, _, _ = self.boxes[self.selected_index]
            self.boxes[self.selected_index] = (class_id, class_name, color, new_x, new_y, w, h)
            self._draw()
            return
        
        # Redimensionnement
        if self.resizing and self.drag_start_pos and self.drag_start_box:
            img_pos = self._display_to_image(self._clamp_to_image(pos))
            self._apply_resize(img_pos)
            self._draw()
    ...
    def mouseReleaseEvent(self, event: QMouseEvent):
        ...
        # Fin déplacement/redim
        if self.dragging or self.resizing:
            if self.selected_index >= 0:
                _, _, _, x, y, w, h = self.boxes[self.selected_index]
                self.box_moved.emit(self.selected_index, x, y, w, h)
            self.dragging = False
            self.resizing = False
            self.resize_handle = HandlePosition.NONE
            self.drag_start_pos = None
    ...
    def _apply_resize(self, img_pos: QPoint):
        ...
        if self.resize_handle == HandlePosition.TOP_LEFT:
            x = min(orig_x + dx, orig_x + orig_w - min_size)
            y = min(orig_y + dy, orig_y + orig_h - min_size)
            w = orig_w - (x - orig_x)
            h = orig_h - (y - orig_y)
        elif self.resize_handle == HandlePosition.TOP_RIGHT:
            y = min(orig_y + dy, orig_y + orig_h - min_size)
            w = max(min_size, orig_w + dx)
            h = orig_h - (y - orig_y)
        elif self.resize_handle == HandlePosition.BOTTOM_LEFT:
            x = min(orig_x + dx, orig_x + orig_w - min_size)
            w = orig_w - (x - orig_x)
            h = max(min_size, orig_h + dy)
        elif self.resize_handle == HandlePosition.BOTTOM_RIGHT:
            w = max(min_size, orig_w + dx)
            h = max(min_size, orig_h + dy)
        elif self.resize_handle == HandlePosition.TOP:
            y = min(orig_y + dy, orig_y + orig_h - min_size)
            h = orig_h - (y - orig_y)
        elif self.resize_handle == HandlePosition.BOTTOM:
            h = max(min_size, orig_h + dy)
        elif self.resize_handle == HandlePosition.LEFT:
            x = min(orig_x + dx, orig_x + orig_w - min_size)
            w = orig_w - (x - orig_x)
        elif self.resize_handle == HandlePosition.RIGHT:
            w = max(min_size, orig_w + dx)
        
        class_id, class_name, color, _, _, _, _ = self.boxes[self.selected_index]
        self.boxes[self.selected_index] = (class_id, class_name, color, x, y, w, h)
...
class ClassListWidget(QWidget):
    ...
    def _create_ui(self):
        ...
        # Liste scrollable avec taille minimale importante
        scroll = QScrollArea()
        scroll.setWidgetResizable(True)
        scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        scroll.setMinimumHeight(250)  # Hauteur minimale pour voir les références
        ...
        layout.addWidget(scroll, stretch=1)  # Stretch pour prendre l'espace disponible
...
class DatasetAnnotatorWidget(QWidget):
    ...
    def _create_ui(self):
        ...
        # Liste des classes - AVEC STRETCH pour prendre l'espace disponible
        ...
        left_layout.addWidget(self.class_list, stretch=1)  # Prend tout l'espace disponible
        
        # Gestion des classes (compact, en bas)
        left_layout.addWidget(self._create_class_management())
        
        # Outils d'annotation avancés (compact, en bas)
    ...
    def _create_tools_group(self):
        ...
        layout.addWidget(btn_auto, 0, 0)
        
        # Ligne 2: Propagation + Interpolation
        btn_tracking = create_tool_btn("🎬 Propager", 
            "Copie les annotations sur N frames\nSimple et rapide!", "#7b1fa2", "#6a1b9a")
        btn_tracking.clicked.connect(self._open_tracking_tool)
        layout.addWidget(btn_tracking, 0, 1)
    ...
    def _open_tracking_tool(self):
        """Ouvrir l'outil de propagation simple - copie les annotations sur N frames"""
        if not self.task.images:
            QMessageBox.warning(self, "Erreur", "Chargez d'abord des images.")
            return
        ...
        if not img or not img.boxes:
            QMessageBox.warning(self, "Erreur",
                ...
                "La propagation copie les annotations de l'image actuelle "
                "aux images suivantes.")
            return
        
        remaining = len(self.task.images) - self.task.current_index - 1
        if remaining <= 0:
            QMessageBox.warning(self, "Erreur", "Pas de frames suivants disponibles.")
            return
        
        # Dialogue simple pour demander le nombre de frames
        num_frames, ok = QInputDialog.getInt(
            self, 
            "🎬 Propagation Simple",
            f"Nombre de frames sur lesquels propager les annotations:\n\n"
            f"• Frame actuel: #{self.task.current_index + 1}\n"
            f"• Annotations à copier: {len(img.boxes)}\n"
            f"• Frames disponibles: {remaining}\n",
            value=min(50, remaining),  # Valeur par défaut
            min=1,
            max=remaining,
            step=1
        )
        ...
        # Copier les annotations sur les N prochains frames
        self._log(f"🎬 Propagation de {len(img.boxes)} annotations sur {num_frames} frames...")
        
        # Récupérer les annotations à copier
        annotations_to_copy = []
        for box in img.boxes:
            annotations_to_copy.append({
        ...
        # Propager sur les frames suivants
        start_frame = self.task.current_index
        propagated_count = 0
        
        for i in range(1, num_frames + 1):
            target_frame = start_frame + i
            if target_frame >= len(self.task.images):
                break
            
            # Aller au frame cible
            self.task.current_index = target_frame
            target_img = self.task.get_current_image()
            
            if target_img:
                # Effacer les annotations existantes si on veut remplacer
                # (ou on pourrait les garder - ici on remplace)
                target_img.boxes.clear()
                ...
                # Sauvegarder
                target_img.save_yolo(self.task.config.output_dir if self.task.config else "")
                propagated_count += 1
        ...
        self._log(f"✅ Propagation terminée: {propagated_count} frames annotés")
        
        QMessageBox.information(self, "✅ Propagation terminée",
            f"Les annotations ont été copiées sur {propagated_count} frames!\n\n"
            f"• Du frame #{start_frame + 1} au frame #{start_frame + propagated_count + 1}\n"
            f"• {len(annotations_to_copy)} annotation(s) par frame\n\n"
            f"💡 Astuce: Utilisez Ctrl+→ dans la vue plein écran\n"
            f"pour propager et ajuster frame par frame.")
...
<h3>🎬 Propagation Simple</h3>
<p>Copie les annotations du frame actuel sur plusieurs frames suivants.</p>
<ul>
<li>Annotez un frame de référence</li>
<li>Cliquez sur Propager et entrez le nombre de frames</li>
<li>Les annotations sont copiées instantanément</li>
<li>Utilisez ensuite <b>Ctrl+→</b> en plein écran pour ajuster</li>
</ul>
<p><b>Gain estimé:</b> 20-50x plus rapide</p>

<h3>🖱️ Drag & Drop (dans Annotation Plein Écran)</h3>
<p>Déplacez et redimensionnez les annotations à la souris.</p>
<ul>
<li><b>Clic</b> sur une annotation → Sélection</li>
<li><b>Glisser</b> une annotation → Déplacement fluide</li>
<li><b>Glisser une poignée</b> → Redimensionnement</li>
<li><b>Ctrl+C</b> → Copier les annotations</li>
<li><b>Ctrl+V</b> → Coller les annotations</li>
<li><b>Ctrl+→</b> → Propager au frame suivant et ajuster</li>
</ul>
...
<ol>
<li>Annotez un frame de référence</li>
<li>Cliquez <b>🎬 Propager</b> sur 30-50 frames</li>
<li>Ouvrez <b>Annotation Plein Écran</b></li>
<li>Naviguez et ajustez les positions avec le drag & drop</li>
<li>Utilisez <b>Ctrl+→</b> pour propager frame par frame si besoin</li>
</ol>
"""
